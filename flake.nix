    ...
    utils.lib.eachSystem [ "x86_64-linux" "i686-linux" "aarch64-linux" ]
      (system:
        let
          ...
          shout = string:
            builtins.replaceStrings [ "-" ] [ "_" ] (nixpkgs.lib.toUpper string);
          ...
          env = {
            ...
            "CARGO_TARGET_${shout rust-target}_LINKER" =
              let inherit (pkgs.pkgsStatic.stdenv) cc;
                ...
                ({ name, partitions }: [
                  {
                    ...
                    command = ''
                      ...
                      # build partitions
                      cargo build --package ${
                        concatStringsSep " " (cargoPackageList partitions)
                      } --target ${rust-target} --release

                      # (build &) run hypervisor
                      RUST_LOG=''${RUST_LOG:=trace} cargo run --package a653rs-linux-hypervisor --release -- examples/${name}.yaml $@
                    '';
                    help = "Run the ${name} example, consisting of the partitions: ${
                  concatStringsSep "," partitions
                }";
                    category = "example";
                  }
                  {
